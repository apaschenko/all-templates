...
class Executor {
    static async parseAndProcessSource(source, startRule, processorData, timeout) {
        ...
        // );
let entryPointAst = parser.parse(source, {startRule})
    ...
    static async runExpression(ast, processorData, timeout) {
        ...
        switch (ast.operator) {
            ...
            case 'prefix ++':
                result = await Executor.setPointerValue(ast.right, ++sources.right, processorData);
                break;
            ...
            case 'postfix ++':
                result = sources.left;
                sources.left++;
                await Executor.setPointerValue(ast.left, sources.left, processorData);
                break;
            ...
            default:
                // TODO: Fire error
                console.log('[ERROR] Expression contains an unknown operator');
        }
