...
class Parser {
    ...
    parse() {
console.log(`Blanks.includes(' '): ${BLANKS.includes(' ') ? 'true' : 'false'}`)
            ...
            switch (status) {
                case STATUS.TEXT:
                    if (symbol === placeholder.open[this.buffer.length]) {
                        if (this.buffer.length === openPhMaxPosition) { // new placeholder started
                            ...
                            this.levels = [];
                            phStartedAt = posInLayer - openPhMaxPosition;
                ...
                case STATUS.PH_REGULAR:
                    ...
                    if ((this.lastCheckedPos < posInLayer)
                        ...
                        if (bufferLength === closePhMaxPosition) {
                            this.buffer = this.result = '';
                            this.delimiter = DELIMITER.TEXT_STARTED;
...
console.log(`===> blanks! prev lexeme: ${this.currentLexeme.getValue()}, lexemeNotStarted = true;`)
                        ...
                        if (lexemeNotStarted) {
console.log(`lexemeNotStarted: prev lexeme: '${this.currentLexeme.getValue()}', symbol: '${symbol}'`)
                            this.addLexemeAndPrepareNew();
...
console.log(`lexemeNotStarted: next lexeme: '${this.currentLexeme.getValue()}', symbol: '${symbol}'`)
                    }
