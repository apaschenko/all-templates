...
class ControlTree {
    ...
    printDebug() {
        ...
        Throw(
            Exceptions.DEBUG,
            `${this.LF}${this.INDENT}` + this.printDebugBlock(this.rootBlock, '')
        );
    }
    ...
    printDebugLexeme(lexeme, indent) {
        ...
        if (lexeme) {
            ...
            let newIndent = `${indent}${this.INDENT}`;
            output = `${this.LF}${indent}[${lexeme.getName()}]${this.LF}`
                + `${indent}value: ${lexeme.getValue()}${this.LF}`;
            prev = lexeme.getPrev();
            output += `${indent}prev: ${this.printName(lexeme.getPrev())}${this.LF}`
                + `${indent}next: ${this.printDebugLexeme(lexeme.getNext(), newIndent)}${this.LF}`
        } else {
            output = '--- missing ---'
        }
        return output;
    ...
    printName(entity) {
        return entity ? entity.getName : '--- missing ---';
    }
}
